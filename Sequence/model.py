...
def RFusNet(R_1, R_2, R_3, training=True):
    with tf.variable_scope('RFusNet', reuse=tf.AUTO_REUSE):
        cat_input=tf.concat([R_1,R_2,R_3],axis=-1)
...
def SFusNet(S_1, S_2, S_3, training=True):
    with tf.variable_scope('SFusNet', reuse=tf.AUTO_REUSE):
        cat_input=tf.concat([S_1,S_2,S_3],axis=-1)
...
def CFusNet(C_1, C_2, C_3, training=True):
    with tf.variable_scope('CFusNet', reuse=tf.AUTO_REUSE):
        cat_input=tf.concat([C_1,C_2,C_3],axis=-1)
