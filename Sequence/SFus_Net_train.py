...
input_S_3 = tf.placeholder(tf.float32, [None, None, None, 1], name='input_S_3')
...
[R_decom_3, C_decom_3, S_decom_3] = DecomNet(input_3,training=False)


#the output of decomposition network
decom_output_S1 = S_decom_1
decom_output_S2 = S_decom_2
decom_output_S3 = S_decom_3

#the output of SFuseNet 
Fus_output_S = SFusNet(input_S_1, input_S_2, input_S_3)
...
input_patch_score = tf.nn.softmax(tf.concat([tf.expand_dims(tf.reduce_mean(score_input_grad_1, axis = [1,2,3])/0.01, axis = -1), tf.expand_dims(tf.reduce_mean(score_input_grad_2,axis = [1,2,3])/0.01, axis = -1), tf.expand_dims(tf.reduce_mean(score_input_grad_3,axis = [1,2,3])/0.01, axis = -1)], axis = -1))
...
#define loss
#SSIM Fusion loss
S_SSIM_loss = tf.reduce_mean(input_1_patch_score*(1-tf_ssim(Fus_output_S, input_S_1))+input_2_patch_score*(1-tf_ssim(Fus_output_S, input_S_2))+input_3_patch_score*(1-tf_ssim(Fus_output_S, input_S_3)))
...
with tf.name_scope('image'): 
  ...
  tf.summary.image('input_S_3',tf.expand_dims(input_S_3[1,:,:,:],0))
...
for idx in range(len(train_3_data)):
    ...
    SS3= sess.run([decom_output_S3], feed_dict={input_3: input_img3})
    decom_output_S3_component = np.squeeze(SS3)
    input_img3_component = np.squeeze(input_img3)
    print(decom_output_S3_component.shape)
    decomposed_3_S_data.append(decom_output_S3_component)
...
train_SFuse_3_S_data = decomposed_3_S_data
...
for epoch in range(start_epoch, epoch):
    for batch_id in range(start_step, numBatch):
        ...
        batch_input_3_S = np.zeros((batch_size, patch_size, patch_size, 1), dtype="float32")
        ...
        for patch_id in range(batch_size):
            ...
            S_3_data = train_SFuse_3_S_data[image_id]
            S_3_expand = np.expand_dims(S_3_data, axis = 2)
            ...
            S_3_data_crop = S_3_expand[x : x+patch_size, y : y+patch_size, :]
            ...
            batch_input_3_S[patch_id, :, :, :] = data_augmentation(S_3_data_crop, rand_mode)
        ...
        counter += 1
        _, loss,summary_str = sess.run([train_op, train_loss,summary_op], feed_dict={input_1: batch_input_1_img, input_2: batch_input_2_img,input_3: batch_input_3_img, input_S_1: batch_input_1_S,input_S_2: batch_input_2_S,input_S_3: batch_input_3_S, lr: learning_rate})
