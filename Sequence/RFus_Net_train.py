...
input_R_3 = tf.placeholder(tf.float32, [None, None, None, 1], name='input_R_3')
...
decom_output_R3 = R_decom_3

#the output of RFuseNet 
Fus_output_R = RFusNet(input_R_1, input_R_2, input_R_3)
...
input_3_gray=tf.image.rgb_to_grayscale(input_3)
input_grad_3_x=tf.abs(gradient_no_norm(low_pass(input_3_v), "x"))
input_grad_3_y=tf.abs(gradient_no_norm(low_pass(input_3_v), "y"))
input_grad_3=input_grad_3_x+input_grad_3_y

gradient_cat = tf.concat([input_grad_1, input_grad_2, input_grad_3],3)
...
#gradient loss
Gra_loss_1 = tf.reduce_mean(input_1_mask*tf.abs(gradient_no_norm_no_abs(Fus_output_R,"x")-gradient_no_norm_no_abs(input_R_1,"x")))+tf.reduce_mean(input_1_mask*tf.abs(gradient_no_norm_no_abs(Fus_output_R,"y")-gradient_no_norm_no_abs(input_R_1,"y")))
Gra_loss_2 = tf.reduce_mean(input_2_mask*tf.abs(gradient_no_norm_no_abs(Fus_output_R,"x")-gradient_no_norm_no_abs(input_R_2,"x")))+tf.reduce_mean(input_2_mask*tf.abs(gradient_no_norm_no_abs(Fus_output_R,"y")-gradient_no_norm_no_abs(input_R_2,"y")))  
Gra_loss_3 = tf.reduce_mean(input_3_mask*tf.abs(gradient_no_norm_no_abs(Fus_output_R,"x")-gradient_no_norm_no_abs(input_R_3,"x")))+tf.reduce_mean(input_3_mask*tf.abs(gradient_no_norm_no_abs(Fus_output_R,"y")-gradient_no_norm_no_abs(input_R_3,"y"))) 


Gra_loss = Gra_loss_1 + Gra_loss_2 + Gra_loss_3
...
with tf.name_scope('image'):
  ...
  tf.summary.image('R_decom_3',tf.expand_dims(R_decom_3[1,:,:,:],0)) 
...
for idx in range(len(train_3_data)):
    ...
    RR3= sess.run([decom_output_R3], feed_dict={input_3: input_img3})
    decom_output_R3_component = np.squeeze(RR3)
    input_img3_component = np.squeeze(input_img3)
    print(decom_output_R3_component.shape)
    decomposed_3_R_data.append(decom_output_R3_component)
...
train_RFuse_3_R_data = decomposed_3_R_data
...
for epoch in range(start_epoch, epoch):
    for batch_id in range(start_step, numBatch):
        ...
        batch_input_3_R = np.zeros((batch_size, patch_size, patch_size, 1), dtype="float32")
        ...
        for patch_id in range(batch_size):
            ...
            R_3_data = train_RFuse_3_R_data[image_id]
            R_3_expand = np.expand_dims(R_3_data, axis = 2)
            ...
            R_3_data_crop = R_3_expand[x : x+patch_size, y : y+patch_size, :]
            ...
            batch_input_3_R[patch_id, :, :, :] = data_augmentation(R_3_data_crop, rand_mode)
        ...
        counter += 1
        _, loss,summary_str = sess.run([train_op, train_loss,summary_op], feed_dict={input_1: batch_input_1_img, input_2: batch_input_2_img, input_3: batch_input_3_img,input_R_1: batch_input_1_R,input_R_2: batch_input_2_R,input_R_3: batch_input_3_R, lr: learning_rate})
