...
input_3 = tf.placeholder(tf.float32, [None, None, None, 3], name='input_3')
...
input_C_3 = tf.placeholder(tf.float32, [None, None, None, 3], name='input_C_3')
...
input_3_hsv = tf.image.rgb_to_hsv(input_3)
input_3_h = tf.expand_dims(input_3_hsv[:,:,:,0],-1)
input_3_s = tf.expand_dims(input_3_hsv[:,:,:,1],-1)
input_3_v = tf.expand_dims(input_3_hsv[:,:,:,2],-1)
...
[R_decom_3, C_decom_3, I_decom_3] = DecomNet(input_3,training=False)

#the output of decomposition network
decom_output_C1 = C_decom_1
decom_output_C2 = C_decom_2
decom_output_C3 = C_decom_3

#the output of CFuseNet 
Fus_output_C = CFusNet(input_C_1, input_C_2, input_C_3)
...
input_C_3_sum=tf.reduce_sum(input_C_3,axis=3,keepdims=True)
input_C_3_vec=tf.div((input_C_3),(input_C_3_sum+0.0001))
...
mask_input_grad_3_x=tf.abs(gradient_no_norm(low_pass(R_decom_3), "x"))
mask_input_grad_3_y=tf.abs(gradient_no_norm(low_pass(R_decom_3), "y"))
mask_input_grad_3=mask_input_grad_3_x+mask_input_grad_3_y


gradient_cat = tf.concat([mask_input_grad_1, mask_input_grad_2, mask_input_grad_3],3)
index_max_init=tf.argmax(gradient_cat,axis=3)
index_max=tf.cast(index_max_init,dtype = tf.float32)
input_1_mask=tf.expand_dims(1-tf.sign(tf.abs(index_max -0)),-1)
input_2_mask= tf.expand_dims(1-tf.sign(tf.abs(index_max -1)),-1)
input_3_mask=1-(input_1_mask+input_2_mask)
...
score_input_grad_3_x=tf.abs(gradient_no_norm(low_pass(input_3_v), "x"))
score_input_grad_3_y=tf.abs(gradient_no_norm(low_pass(input_3_v), "y"))
score_input_grad_3=score_input_grad_3_x+score_input_grad_3_y

input_patch_score = tf.nn.softmax(tf.concat([tf.expand_dims(tf.reduce_mean(score_input_grad_1, axis = [1,2,3])/0.08, axis = -1), tf.expand_dims(tf.reduce_mean(score_input_grad_2,axis = [1,2,3])/0.08, axis = -1), tf.expand_dims(tf.reduce_mean(score_input_grad_3,axis = [1,2,3])/0.08, axis = -1)], axis = -1), axis = -1)
input_1_patch_score = input_patch_score[:,0:1]
input_2_patch_score = input_patch_score[:,1:2]
input_3_patch_score = input_patch_score[:,2:3]
...
#Fusion loss
#Intensity loss
NCV_loss = tf.reduce_mean(input_1_mask*tf.abs(Fuse_C_vec - input_C_1_vec)) + tf.reduce_mean(input_2_mask*tf.abs(Fuse_C_vec - input_C_2_vec)) + tf.reduce_mean(input_3_mask*tf.abs(Fuse_C_vec - input_C_3_vec))

#SSIM Fusion loss
S_SSIM_loss_1 = input_1_patch_score*(1-tf_ssim(Fus_output_C[:,:,:,0:1], input_C_1[:,:,:,0:1]))+input_2_patch_score*(1-tf_ssim(Fus_output_C[:,:,:,0:1], input_C_2[:,:,:,0:1]))+input_3_patch_score*(1-tf_ssim(Fus_output_C[:,:,:,0:1], input_C_3[:,:,:,0:1]))
S_SSIM_loss_2 = input_1_patch_score*(1-tf_ssim(Fus_output_C[:,:,:,1:2], input_C_1[:,:,:,1:2]))+input_2_patch_score*(1-tf_ssim(Fus_output_C[:,:,:,1:2], input_C_2[:,:,:,1:2]))+input_3_patch_score*(1-tf_ssim(Fus_output_C[:,:,:,1:2], input_C_3[:,:,:,1:2]))
S_SSIM_loss_3 = input_1_patch_score*(1-tf_ssim(Fus_output_C[:,:,:,2:3], input_C_1[:,:,:,2:3]))+input_2_patch_score*(1-tf_ssim(Fus_output_C[:,:,:,2:3], input_C_2[:,:,:,2:3]))+input_3_patch_score*(1-tf_ssim(Fus_output_C[:,:,:,2:3], input_C_3[:,:,:,2:3]))
...
with tf.name_scope('scalar'):
  ...
  tf.summary.scalar('input_3_patch_score',tf.reduce_mean(input_3_patch_score))
...
with tf.name_scope('image'):
  ...
  tf.summary.image('input_3_mask',tf.expand_dims(input_3_mask[1,:,:,:],0))
...
train_1_data_names = glob(args.train_data_dir +'/input_1/*')
train_1_data_names.sort()

train_2_data_names = glob(args.train_data_dir +'/input_2/*') 
train_2_data_names.sort()

train_3_data_names = glob(args.train_data_dir +'/input_3/*') 
train_3_data_names.sort()

assert len(train_1_data_names) == len(train_2_data_names) == len(train_3_data_names)
...
for idx in range(len(train_1_data_names)):
    ...
    im_3 = load_images_no_norm(train_3_data_names[idx])
    train_3_data.append(im_3)
...
for idx in range(len(train_3_data)):
    input_img3 = np.expand_dims(train_3_data[idx], axis=0)
    CC3= sess.run([decom_output_C3], feed_dict={input_3: input_img3})
    decom_output_C3_component = np.squeeze(CC3)
    input_img3_component = np.squeeze(input_img3)
    decomposed_3_C_data.append(decom_output_C3_component)
    input_3_img_data.append(input_img3_component)


train_CFuse_1_C_data = decomposed_1_C_data
train_CFuse_2_C_data = decomposed_2_C_data
train_CFuse_3_C_data = decomposed_3_C_data
train_Decom_1_img_data = input_1_img_data
train_Decom_2_img_data = input_2_img_data
train_Decom_3_img_data = input_3_img_data
...
for epoch in range(start_epoch, epoch):
    for batch_id in range(start_step, numBatch):
        ...
        batch_input_3_img = np.zeros((batch_size, patch_size, patch_size, 3), dtype="float32")
        ...
        batch_input_3_C = np.zeros((batch_size, patch_size, patch_size, 3), dtype="float32")
        ...
        for patch_id in range(batch_size):
            ...
            batch_input_3_C[patch_id, :, :, :] = data_augmentation(C_3_data_crop, rand_mode)
            ...
            batch_input_3_img[patch_id, :, :, :] = data_augmentation(img_3_data_crop, rand_mode)


            image_id = (image_id + 1) % len(train_CFuse_1_C_data)
        counter += 1
        _, loss,summary_str = sess.run([train_op, train_loss,summary_op], feed_dict={input_1: batch_input_1_img, input_2: batch_input_2_img,input_3: batch_input_3_img,input_C_1: batch_input_1_C,input_C_2: batch_input_2_C,input_C_3: batch_input_3_C, lr: learning_rate})
